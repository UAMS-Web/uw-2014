...
UAMS.elements = {
  ...
  mobilemenu : '.uams-mobile-menu-toggle',
  radio      : ':radio',
  checkbox   : ':checkbox',
  search     : '.uams-search',
  select     : '.uams-select',
...
UAMS.baseUrl = Backbone.history.location.origin +
             Backbone.history.location.pathname

UAMS.sources = {
  quicklinks : UAMS.baseUrl + 'wp-admin/admin-ajax.php?action=quicklinks',
  search     : UAMS.baseUrl + 'wp-admin/admin-ajax.php'
}
...
UAMS.initialize = function( $ )
{
  ...
  UAMS.search     = _.map( $( UAMS.elements.search ),    function( element ) { return new UAMS.Search( { el : element, model : new UAMS.Search.DirectoryModel( {url: UAMS.sources.search}) }) } )
...
jQuery(document).ready( UAMS.initialize )
...
UAMS.Search = Backbone.View.extend({
  ...
  searchbar : '<div id="uamssearcharea" class="uams-search-bar-container">'+
               '<div class="container">'+
                  '<div class="center-block uams-search-wrapper">'+
                    '<form class="uams-search" action="<%= Backbone.history.location.pathname %>">'+
                      '<input id="uams-search-bar" type="search" name="s" value="" autocomplete="off" tabindex="-1"/>'+
                    '</form>'+
                ...
                '<div class="uams-results" style="display:none;">' +
  ...
  events :
  {
    ...
    'keyup #uams-search-bar'      : 'searchDirectory',
  ...
  initialize :function ( options )
  {
    ...
    this.$results  = this.$( '.uams-results' )
  ...
  toggleBlur: function()
  {
    if (this.$searchbar.hasClass('open')) {
        this.$searchbar.find('#uams-search-bar').focus();
    }
  },
  ...
  keyDownDispatch: function(event)
  {
    if (event.keyCode == 27){
        ...
        else if ($target.is('#uams-search-bar')){
            if (event.keyCode == 9) {
                event.preventDefault();
  ...
  toggleSearchFeature : function( e )
  {
    ...
    _.defer(function($searchbar) { $searchbar.find('#uams-search-bar').focus() }, this.$searchbar);
  ...
  submitSearch : function( e )
  {
    switch ( this.searchFeature )
    {
      case this.searchFeatures.uams :
        ...
        this.$searchbar.find('form').attr('action', 'http://uams.edu/search/')
        return true;
...
UAMS.MobileMenu = Backbone.View.extend({
  ...
  {
    this.settings = _.extend( {}, this.defaults , this.$el.data() , options )
    this.$mobilemenu = $('.uams-mobile-menu > li')
  },
